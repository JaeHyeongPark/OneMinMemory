    ...
    socket.on("disconnect", async () => {
      try {
        ...
        if (permissionUser === socket.Id) {
          ...
          await redis.v4.expire(socket.roomId + "playlistPermissionState",21600)
          status = 0;
        }
        ...
        if (myVoteState === "true") {
          ...
          renderVoteState = await redis.v4.decr(socket.roomId + "/renderVoteState")
    ...
    socket.on("IVoted", async (data) => {
      try {
        ...
        if (data.voteState === true) {
          ...
          renderVoteState = await redis.v4.incr(data.roomId + "/renderVoteState")
          ...
          renderVoteState = await redis.v4.decr(data.roomId + "/renderVoteState")
          redis.v4.expire(data.roomId + "/renderVoteState", 21600);
